...
<?php
if(!empty($productID)){
$productQ = "SELECT * FROM  products_range  WHERE productrange_id='$productID' ORDER BY productrange_id LIMIT 1";
$productQ_data = $data->con->query($productQ);
if ($productQ_data->num_rows > 0) {
foreach($productQ_data as $productRow)
{
?>
...
<input type="hidden" id="producimgtID" name="producimgtID" value="<?php echo $productRow['productrange_id']; ?>" />
<!--- process -->
<ul class="list-unstyled mb-0" id="dropzone-preview">
<?php
$prodImageQ = "SELECT * FROM product_images WHERE productrange_id='".$productRow['productrange_id']."'";
$prodImageQ_data = $data->con->query($prodImageQ);
foreach($prodImageQ_data as $proimgRow)
{
?>
<li class="mt-2" >
...
<img data-dz-thumbnail class="img-fluid rounded d-block" src="<?php echo pathUrl(__DIR__ . '/../../../../'); ?>uploads/product/<?php echo $proimgRow['productimages_name']; ?>" />
</div>
</div>
...
<button type="button" data-dz-remove id="<?php echo $proimgRow['productimages_id']; ?>" class="btn btn-sm btn-danger ImageDel">Delete</button>
</div>
</div>
</div>
</li>
...
<div class="mb-3">
<input type="hidden" id="productID" value="<?php echo $productRow['productrange_id']; ?>" />
<input type="text" id="productName" value="<?php echo $productRow['productrange_name']; ?>" class="required form-control" placeholder="Product Range Name" />
</div>
<div class="mb-3">
<div id="productContent"><?php echo $productRow['productrange_content']; ?></div>
</div>
<div class="mb-3">
<select class="required form-control" id="productStatus">
<option value="publish">Publish</option>
<option value="unpublish">Unpublish</option>
</select>
</div>
<?php } } else {
header("location:pathUrl__DIR__ . /../products/new/range");
} } else { ?>
<div class="mb-3">
<input type="text" id="productName" class="required form-control" placeholder="Product  Name" />
</div>
<div class="mb-3">
...
method:"post",
data:{productID:productID,productName:productName,productContent:productContent,productStatus:productStatus},
dataType:"text",
success:function(data){
var check = ['This value is required','Name Duplicate'];
if(check.includes(data)){
$("#productName").css("border-color","#FF5733");
$("#productContent").css("border-color","#FF5733");
$("#productStatus").css("border-color","#FF5733");
toastr.error(data);
}else if(data == 'Update'){
toastr.info(data);
...
// product range images delete
$('.ImageDel').click( function(){
var productimagedelID   = $(this).attr("id");
$.ajax({
url: baseUrl + urldelect + baseUrlformat,
method:"post",
data:{productimagedelID:productimagedelID},
dataType:"text",
success:function(data){
toastr.error(data);
setTimeout(function(){ location.reload(); }, 3000);
...
urlupload = "/template/function/upload/content.gallery";
...
<script>
$(document).ready(function(){
let loadUrl       = '/template/content/product/product.content';
...
$(document).ready(function(){
var uploadUrl     = '/template/function/upload/product.image';
