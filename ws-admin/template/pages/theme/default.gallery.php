...
let loadUrl       = '/template/content/theme/default.gallery';
...
{
load_data(1);
function load_data(page, query = '', update = '<?php if(!empty($galleryID)){ echo $galleryID; }else{ echo $galleryID = ''; } ?>')
{
$.ajax({
url: baseUrl + loadUrl + baseUrlformat,
method:"POST",
data:{page:page, query:query, update:update},
...
$(document).on('change', '.menusearch', function(){
$('.submenu').css('opacity','0.1');
