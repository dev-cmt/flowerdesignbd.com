...
<?php
if(!empty($projectID)){
$projectQ = "SELECT * FROM projects  WHERE project_id='$projectID' ORDER BY project_id LIMIT 1";
$projectQ_data = $data->con->query($projectQ);
if ($projectQ_data->num_rows > 0) {
...
<input type="hidden" id="projectID" name="projectID" value="<?php echo $proRow['project_id']; ?>" />
<!--- process -->
<ul class="list-unstyled mb-0" id="dropzone-preview">
<?php
$proImageQ = "SELECT * FROM projects_images WHERE project_id='".$proRow['project_id']."'";
...
<img data-dz-thumbnail class="img-fluid rounded d-block" src="<?php echo pathUrl(__DIR__ . '/../../../../'); ?>uploads/project/<?php echo $primgRow['projectimages_name']; ?>" />
</div>
</div>
...
<button type="button" data-dz-remove id="<?php echo $primgRow['projectimages_id']; ?>" class="btn btn-sm btn-danger ImageDel">Delete</button>
</div>
</div>
</div>
</li>
...
<div class="mb-3">
<input type="hidden" id="projectID" value="<?php echo $proRow['project_id']; ?>" />
<input type="text" id="projectName" value="<?php echo $proRow['project_name']; ?>" class="required form-control" placeholder="Projects Name" />
</div>
<div class="mb-3">
<div id="projectContent"><?php echo $proRow['project_content']; ?></div>
</div>
<div class="mb-3">
<select class="required form-control" id="projectStatus">
...
<div class="mb-3">
<input type="text" id="projectName" class="required form-control" placeholder="Projects Name" />
</div>
<div class="mb-3">
...
method:"post",
data:{projectID:projectID,projectName:projectName,projectContent:projectContent,projectStatus:projectStatus},
dataType:"text",
success:function(data){
var check = ['This value is required','Name Duplicate'];
if(check.includes(data)){
$("#projectName").css("border-color","#FF5733");
$("#projectContent").css("border-color","#FF5733");
$("#projectStatus").css("border-color","#FF5733");
toastr.error(data);
}else if(data == 'Update'){
toastr.info(data);
...
var proimagedelID   = $(this).attr("id");
$.ajax({
url: baseUrl + urldelect + baseUrlformat,
method:"post",
data:{proimagedelID:proimagedelID},
dataType:"text",
success:function(data){
toastr.error(data);
setTimeout(function(){ location.reload(); }, 3000);
...
urlupload = "/template/function/upload/project.gallery";
...
<script>
$(document).ready(function(){
let loadUrl       = '/template/content/project/project.content';
...
$(document).ready(function(){
var uploadUrl     = '/template/function/upload/project.image';
